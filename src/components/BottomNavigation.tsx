...
const BottomNavigation: React.FC<BottomNavigationProps> = ({ activeTab, onTabChange }) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-2">
      <button 
        className={`flex flex-col items-center p-2 ${activeTab === 'home' ? 'text-orange-400' : 'text-gray-500'}`}
      ...
      <button 
        className={`flex flex-col items-center p-2 ${activeTab === 'questions' ? 'text-orange-400' : 'text-gray-500'}`}
        onClick={() => onTabChange('questions')}
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
        </svg>
        <span className="text-xs">My Questions</span>
      </button>
    </div>
  );
};
